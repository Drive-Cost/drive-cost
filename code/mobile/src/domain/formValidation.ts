...
function decimal(value: string, label: string, minimum: number): ValidationResult<number> {
  ...
  if (!value.trim() || !Number.isFinite(parsed) || parsed < minimum) {
    return {
      ok: false,
      error: `${label} must be ${minimum === 0 ? "zero or greater" : "greater than zero"}.`,
    };
  }
...
export function validateVehicleForm(input: {
  ...
  const year = wholeNumber(input.year, "Year", 1886);
  const maximumYear = new Date().getFullYear() + 1;
  if (!year.ok || year.value > maximumYear) {
    return { ok: false, error: `Year must be between 1886 and ${maximumYear}.` };
  }

  const ownershipStartMileage = wholeNumber(
    input.ownershipStartMileage,
    "Ownership start mileage",
    0,
  );
  if (!ownershipStartMileage.ok) return ownershipStartMileage;

  const trackingStartMileage = wholeNumber(
    input.trackingStartMileage,
  ...
  const powerHp = input.powerHp.trim()
    ? wholeNumber(input.powerHp, "Power", 1)
    : undefined;
...
export function validateEnergyEntryForm(input: {
  ...
  const quantity = decimal(input.quantity, "Energy quantity", 0.000001);
...
export function validateMaintenanceEntryForm(input: {
  ...
  return {
    ok: true,
    value: { type: type.value, cost: cost.value, odometer: odometer.value },
  };
}
